...
def fxTwoDevHighSpeedTest(port1, port2, baudRate, controllerType = Controller.position, signalType = signal.sine, commandFreq = 1000, signalAmplitude = 10000, numberOfLoops = 10, signalFreq = 1, cycleDelay = .1, requestJitter = False, jitter = 200):
	...
	try:
		...
		stream1 = Stream(port1, baudRate, varsToStream, 2, labels, streamFreq, shouldLog, shouldAutostream)
		stream2 = Stream(port2, baudRate, varsToStream, 2, labels, streamFreq, shouldLog, shouldAutostream)
		sleep(0.4)
		...
		if (controllerType == Controller.current):
			setControlMode(stream1.devId, CTRL_CURRENT)
			setControlMode(stream2.devId, CTRL_CURRENT)
			print("Setting up current control demo")
		elif (controllerType == Controller.position):
			setControlMode(stream1.devId, CTRL_POSITION)
			setControlMode(stream2.devId, CTRL_POSITION)
			print("Setting up position control demo")
			initial_pos1 = stream1([FX_ENC_ANG])
			initial_pos2 = stream2([FX_ENC_ANG])

		else:
			assert 0	
					
		setGains(stream1.devId, 300, 50, 0, 0)
		setGains(stream2.devId, 300, 50, 0, 0)
		...
		for reps in range(0, numberOfLoops):
			for sample in samples:
				...
				# read ActPack data
				if (controllerType == Controller.current):
					setMotorCurrent(stream1.devId, sample)
					setMotorCurrent(stream2.devId, sample)
					data1 = stream1([FX_MOT_CURR])
					data2 = stream2([FX_MOT_CURR])

				elif (controllerType == Controller.position):
					setPosition(stream1.devId, sample) 
					setPosition(stream2.devId, sample) 
					data1 = stream1([FX_ENC_ANG])
					data2 = stream2([FX_ENC_ANG])

				measurements1.append(data1)
				measurements2.append(data2)
		...
		setControlMode(stream1.devId, CTRL_NONE)
		setControlMode(stream2.devId, CTRL_NONE)
		...
		if (controllerType == Controller.current):
			plt.figure(1)
			title = "Current control device 1 with " + "{:.2f}".format(actual_frequency) + " Hz, " + \
			...
			plt.figure(2)
			title = "Current control device 2 with " + "{:.2f}".format(actual_frequency) + " Hz, " + \
				str(signalAmplitude) + " mA amplitude " + signalTypeStr + " and " + "{:.2f}".format(command_frequency) + " Hz commands"
			plt.plot(times, requests, color = 'b', label = 'desired current')
			plt.plot(times, measurements2, color = 'r', label = 'measured current')
			plt.xlabel("time (s)")
			plt.ylabel("motor current (mA)")
			...
			plt.figure(1)
			title = "Position control device 1 with " + "{:.2f}".format(actual_frequency) + " Hz, " + \
			...
			plt.figure(2)
			title = "Position control device 2 with " + "{:.2f}".format(actual_frequency) + " Hz, " + \
				str(signalAmplitude) + " amplitude " + signalTypeStr + " and " + "{:.2f}".format(command_frequency) + " Hz commands"
			plt.plot(times, requests, color = 'b', label = 'desired position')
			plt.plot(times, measurements2, color = 'r', label = 'measured position')
			plt.xlabel("time (s)")
			plt.ylabel("Encoder position")
...
if __name__ == '__main__':
	...
	ports = sys.argv[2:3]
	try:
		fxTwoDevHighSpeedTest(ports[0], ports[1], baudRate)
	except Exception as e:
		print("broke: " + str(e))
